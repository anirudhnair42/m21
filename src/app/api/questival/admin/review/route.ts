import { getSettings, handler, json, mergedCatalog, must, personMap, personOf, questMap, requireOrganizer } from "@/lib/forum-server";
...
export const GET = handler(async (request) => {
  ...
  const [subs, finals, quests, settings] = await Promise.all([
  ...
  const [submissions, people] = await Promise.all([
    submissionDTOs(supabase, (must(subs) ?? []) as SubmissionRow[], questMap(quests), { settings, viewer: { id: null, organizer: true } }),
